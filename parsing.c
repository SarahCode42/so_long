...
/*   Updated: 2025/03/19 11:36:13 by jbensimo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_walls(t_game *g)
{
	...
	while (i < g->parsing->height)
	{
		...
		if_not((void *)(long)(g->parsing->map[i][0] == '1' \
			&& g->parsing->map[i][g->parsing->width - 1] \
			== '1'), "Map must be enclosed by walls\n", g);
		i++;
	}
	j = 0;
	while (j < g->parsing->width)
	{
		if_not((void *)(long)(g->parsing->map[0][j] \
			== '1' && g->parsing->map[g->parsing->height - 1][j] \
			== '1'), "Map must be enclosed by walls\n", g);
		j++;
	}
}
...
int	check_elements(t_game *g)
{
	...
	if_not((void *)(long)(g->parsing->player_count == 1 \
		&& g->parsing->exit_count == 1 && g->parsing->collect_count > 0), \
...
void	check_extension(char *filename, t_game *g)
{
	...
	if_not((void *)(long)(len >= 4 && ft_strncmp(filename + len - 4, ".ber", 4) \
	== 0), "Map file must have a .ber extension\n", g);
}
...
void	parsing(t_game *g)
{
	...
	load_map(g);
	check_walls(g);
	if_not((void *)(long)(check_elements(g)),
		"Map must have 1 'P', 1 'E', and at least 1 'C'\n", g);
