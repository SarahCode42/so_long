...
/*   Updated: 2025/03/18 16:37:34 by jbensimo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_visited(t_game *g)
{
	...
	if_not(g->parsing->visited, "Alloc failed for visited\n", g);
	i = 0;
	while (i < g->parsing->height)
	{
		g->parsing->visited[i] = ft_calloc(g->parsing->width, sizeof(int));
		if_not(g->parsing->visited[i], "Alloc failed for visited row\n", g);
		i++;
	}
}
...
{
	if_not((void *)(long)
		(g->parsing->collectibles_found == g->parsing->collect_count),
